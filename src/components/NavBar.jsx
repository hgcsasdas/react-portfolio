...
const NavBar = () => {
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 px-4 text-white bg-blue-600 fixed">
      <div>
        <h1 className="text-5xl font-signature ml-2">Hgc</h1>
      </div>
      ...
      <div>
        <ul className="hidden md:flex">
          {links.map(({ id, link }) => (
            <li
              key={id}
              className="px-4 cursor-pointer capitalize font-bold text-blue-100 hover:scale-105 duration-200"
            >
      ...
      {nav && (
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-blue-600 to-blue-900 text-blue-100 ">
          {links.map(({ id, link }) => (
            ...
            >
              <Link onClick={() => setNav(!nav)} to={link} smooth={true} duration={400}>
                {link}
              </Link>{" "}
            </li>
          ))}
